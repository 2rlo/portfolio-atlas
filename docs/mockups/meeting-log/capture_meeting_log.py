...
BASE_URL = "http://127.0.0.1:4173/what/meeting-log"
...
def run() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        assert page.locator("h1").inner_text() == "MEETING\nLOG"
        ...
        assert "Synced meeting record and source sections 설계 설명 보기" in snapshot
        assert "Editable candidate with source evidence and human review actions 설계 설명 보기" in snapshot
        ...
        activate(page, "source-record", "03-source-record-active.png", "무엇을 읽었는지")
        activate(page, "change-guard", "04-change-guard-active.png", "매일 다시 해석")
        activate(page, "candidate-group", "05-ai-candidate-active.png", "pending 상태")
        activate(page, "human-review", "06-human-review-active.png", "수정·승인·반려")
        activate(page, "capture-boundary", "07-capture-boundary-active.png", "막힌 upstream")

        workflow_meeting = page.locator('button[aria-label^="회의록:"]')
        workflow_meeting.focus()
        assert page.locator('[data-hotspot="source-record"]').get_attribute("aria-pressed") == "true"
        ...
        mobile_page.locator('[data-hotspot="human-review"]').tap()
        mobile_page.locator("#product-editorial-note").evaluate(
            "element => element.scrollIntoView({ block: 'start' })"
        )
        mobile_page.wait_for_timeout(80)
        assert "수정·승인·반려" in mobile_page.locator("#product-editorial-note").inner_text()
        ...
        duration = reduced_page.locator('[data-hotspot="human-review"]').evaluate(
