...
BASE_URL = "http://127.0.0.1:4173/what/developer-status"
...
def run() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        assert page.locator("h1").inner_text() == "DEVELOPER\nSTATUS"
        ...
        assert "Explicit person and account scope 설계 설명 보기" in snapshot
        assert "Predictable snapshot refresh cadence 설계 설명 보기" in snapshot
        ...
        activate(page, "identity-scope", "03-identity-scope-active.png", "추측으로 연결")
        activate(page, "source-hierarchy", "04-source-hierarchy-active.png", "같은 사실")
        activate(page, "grouped-evidence", "05-grouped-evidence-active.png", "커밋 수보다")
        activate(page, "partial-state", "06-partial-state-active.png", "수집된 근거")
        activate(page, "refresh-cadence", "07-refresh-cadence-active.png", "예측 가능한 갱신")
        ...
        mobile_page.locator('[data-hotspot="partial-state"]').tap()
        mobile_page.locator("#product-editorial-note").evaluate("element => element.scrollIntoView({ block: 'start' })")
        mobile_page.wait_for_timeout(80)
        assert "수집된 근거" in mobile_page.locator("#product-editorial-note").inner_text()
        ...
        duration = reduced_page.locator('[data-hotspot="identity-scope"]').evaluate("element => getComputedStyle(element).transitionDuration")
        assert duration == "0s"
        reduced.close()
