...
BASE_URL = "http://127.0.0.1:4173/what/api-usage"
...
def run() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        assert page.locator("h1").inner_text() == "API\nUSAGE"
        ...
        assert "Call-level usage and disconnect ledger 설계 설명 보기" in snapshot
        assert "Workload-specific background budget guard 설계 설명 보기" in snapshot
        ...
        activate(page, "usage-ledger", "03-usage-ledger-active.png", "정상 종료되지 않은")
        activate(page, "pricing-dimensions", "04-pricing-dimensions-active.png", "하나의 단가")
        activate(page, "workload-split", "05-workload-split-active.png", "비용을 만드는 기능")
        activate(page, "budget-guard", "06-budget-guard-active.png", "사용자 채팅")
        activate(page, "counterfactual", "07-counterfactual-active.png", "같은 금액")
        ...
        mobile_page.locator('[data-hotspot="budget-guard"]').tap()
        mobile_page.locator("#product-editorial-note").evaluate(
            "element => element.scrollIntoView({ block: 'start' })"
        )
        mobile_page.wait_for_timeout(80)
        assert "사용자 채팅" in mobile_page.locator("#product-editorial-note").inner_text()
        ...
        duration = reduced_page.locator('[data-hotspot="usage-ledger"]').evaluate(
