...
BASE_URL = "http://127.0.0.1:4173/what/rag-assistant"
...
def run() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        assert page.locator("h1").inner_text() == "RAG\nASSISTANT"
        ...
        assert "Deterministic status lookup before vector retrieval 설계 설명 보기" in snapshot
        assert "Insufficient relationship evidence answer state 설계 설명 보기" in snapshot
        ...
        activate(page, "context-entry", "03-context-entry-active.png", "데이터 구조")
        activate(page, "deterministic-first", "04-deterministic-first-active.png", "vector 검색")
        activate(page, "source-hierarchy", "05-source-hierarchy-active.png", "검색 점수")
        activate(page, "insufficient-evidence", "06-insufficient-evidence-active.png", "부족함 자체")
        activate(page, "review-boundary", "07-review-boundary-active.png", "공식 기록")
        ...
        mobile_page.locator('[data-hotspot="insufficient-evidence"]').tap()
        mobile_page.locator("#product-editorial-note").evaluate(
            "element => element.scrollIntoView({ block: 'start' })"
        )
        mobile_page.wait_for_timeout(80)
        assert "부족함 자체" in mobile_page.locator("#product-editorial-note").inner_text()
        ...
        duration = reduced_page.locator('[data-hotspot="source-hierarchy"]').evaluate(
