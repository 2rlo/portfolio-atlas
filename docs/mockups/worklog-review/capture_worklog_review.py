...
BASE_URL = "http://127.0.0.1:4173/what/worklog-review"
...
def run() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        print({
            ...
            "related_links": page.locator(".wl-related > a").count(),
        })
        assert page.locator("h1").inner_text() == "WORKLOG\nREVIEW"
        assert page.locator("[data-hotspot]").count() == 5
        assert page.locator("button button").count() == 0
        assert page.locator(".wl-related > a").count() == 2
        assert page.locator(".wl-related > div").count() == 1
        assert_no_horizontal_overflow(page, "desktop")

        accessibility_snapshot = page.locator(".product-inspection-frame").aria_snapshot()
        assert "Original worklog source continuity 설계 설명 보기" in accessibility_snapshot
        assert "Weekly report source eligibility boundary 설계 설명 보기" in accessibility_snapshot
        ...
        activate_and_capture(page, "source-continuity", "03-source-continuity-active.png", "원문으로 돌아갈 길")
        activate_and_capture(page, "structured-draft", "04-structured-draft-active.png", "검토 가능한 구조")
        activate_and_capture(page, "ambiguity-level", "05-ambiguity-active.png", "사람이 답할 질문")
        activate_and_capture(page, "human-correction", "06-human-correction-active.png", "최초 workflow의 authority")
        activate_and_capture(page, "report-boundary", "07-report-boundary-active.png", "예외는 숨기지 않는다")
        page.locator(".wl-workflow-section").screenshot(path=str(OUTPUT_DIR / "08-workflow.png"))
        page.locator(".wl-evolution").screenshot(path=str(OUTPUT_DIR / "09-evolution.png"))
        page.locator(".wl-evidence").screenshot(path=str(OUTPUT_DIR / "10-evidence-status.png"))
        page.locator(".wl-boundary").screenshot(path=str(OUTPUT_DIR / "10-boundary.png"))
        ...
        mobile_page.locator(".product-inspection-surface").evaluate("element => element.scrollIntoView({ block: 'start' })")
        mobile_page.wait_for_timeout(80)
        mobile_page.screenshot(path=str(OUTPUT_DIR / "11-mobile-product-view.png"))
        mobile_page.locator('[data-hotspot="ambiguity-level"]').tap()
        mobile_page.locator("#product-editorial-note").evaluate("element => element.scrollIntoView({ block: 'start' })")
        mobile_page.wait_for_timeout(80)
        assert "사람이 답할 질문" in mobile_page.locator("#product-editorial-note").inner_text()
        ...
        transition_duration = reduced_page.locator('[data-hotspot="source-continuity"]').evaluate(
