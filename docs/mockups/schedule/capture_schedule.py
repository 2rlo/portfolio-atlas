...
BASE_URL = "http://127.0.0.1:4173/what/schedule"
...
def run() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        assert page.locator("h1").inner_text() == "SCHEDULE\nOVERVIEW"
        ...
        assert "Canonical plan and actual timeline 설계 설명 보기" in snapshot
        assert "Shared Schedule read-only role boundary 설계 설명 보기" in snapshot
        ...
        activate(page, "canonical-timeline", "03-plan-actual-active.png", "같은 상태가 아니다")
        activate(page, "missing-data", "04-missing-data-active.png", "비어 있다는 사실")
        activate(page, "staged-changes", "05-staged-change-active.png", "검토할 proposal")
        activate(page, "audit-revert", "06-audit-revert-active.png", "최신 변경 확인")
        activate(page, "role-boundary", "07-role-boundary-active.png", "같은 행동")
        ...
        mobile_page.locator('[data-hotspot="role-boundary"]').tap()
        mobile_page.locator("#product-editorial-note").evaluate("element => element.scrollIntoView({ block: 'start' })")
        mobile_page.wait_for_timeout(80)
        assert "같은 행동" in mobile_page.locator("#product-editorial-note").inner_text()
        ...
        duration = reduced_page.locator('[data-hotspot="canonical-timeline"]').evaluate("element => getComputedStyle(element).transitionDuration")
        assert duration == "0s"
        reduced.close()
