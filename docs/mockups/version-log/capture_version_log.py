...
BASE_URL = "http://127.0.0.1:4173/what/version-log"
...
def run() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        assert page.locator("h1").inner_text() == "VERSION\nLOG"
        ...
        assert "Explicit release status filter instead of version name inference 설계 설명 보기" in snapshot
        assert "Deployment caution kept separate from updates and implementation evidence 설계 설명 보기" in snapshot
        ...
        activate(page, "release-status", "03-release-status-active.png", "별도 상태 필드")
        activate(page, "same-date-builds", "04-same-date-builds-active.png", "하나를 지우지 않고")
        activate(page, "compatibility-scope", "05-compatibility-active.png", "모든 장비")
        activate(page, "deployment-note", "06-deployment-note-active.png", "적용 전에")
        activate(page, "source-trace", "07-source-trace-active.png", "어떤 version record")

        workflow_filter = page.locator('button[aria-label^="FILTER:"]')
        workflow_filter.focus()
        assert page.locator('[data-hotspot="release-status"]').get_attribute("aria-pressed") == "true"
        ...
        mobile_page.locator('[data-hotspot="deployment-note"]').tap()
        mobile_page.locator("#product-editorial-note").evaluate(
            "element => element.scrollIntoView({ block: 'start' })"
        )
        mobile_page.wait_for_timeout(80)
        assert "적용 전에" in mobile_page.locator("#product-editorial-note").inner_text()
        ...
        duration = reduced_page.locator('[data-hotspot="release-status"]').evaluate(
