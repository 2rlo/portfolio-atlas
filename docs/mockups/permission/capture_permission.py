...
BASE_URL = "http://127.0.0.1:4173/what/permission"
...
def run() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        assert page.locator("h1").inner_text() == "PERMISSION\nCONTROL"
        ...
        assert "Authenticated identity and application access linkage 설계 설명 보기" in snapshot
        assert "Resource by action effective permission matrix shared by UI and API 설계 설명 보기" in snapshot
        ...
        activate(page, "identity-link", "03-identity-link-active.png", "인증됐다는 사실")
        activate(page, "role-default", "04-role-default-active.png", "반복되는 기본값")
        activate(page, "personal-override", "05-personal-override-active.png", "달라진 resource")
        activate(page, "effective-permission", "06-effective-permission-active.png", "직접 URL")
        activate(page, "protected-change", "07-protected-change-active.png", "복구 가능한 관리자")
        ...
        mobile_page.locator('[data-hotspot="protected-change"]').tap()
        mobile_page.locator("#product-editorial-note").evaluate(
            "element => element.scrollIntoView({ block: 'start' })"
        )
        mobile_page.wait_for_timeout(80)
        assert "복구 가능한 관리자" in mobile_page.locator("#product-editorial-note").inner_text()
        ...
        duration = reduced_page.locator('[data-hotspot="effective-permission"]').evaluate(
