...
BASE_URL = "http://127.0.0.1:4173/what/ai-candidate-review"
...
def run() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        print(
            {
                ...
                "related_links": page.locator(".acr-related > a").count(),
            }
        )
        assert page.locator("h1").inner_text() == "AI CANDIDATE\nREVIEW"
        assert page.locator("[data-hotspot]").count() == 5
        assert page.locator("button button").count() == 0
        assert page.locator(".acr-related > a").count() == 2
        assert page.locator(".acr-related > div").count() == 1
        assert_no_horizontal_overflow(page, "desktop")

        accessibility_snapshot = page.locator(
            ".product-inspection-frame"
        ).aria_snapshot()
        assert "Pending review queue 설계 설명 보기" in accessibility_snapshot
        assert "Human approval decision 설계 설명 보기" in accessibility_snapshot
        assert "WORKSPACE" not in accessibility_snapshot

        for path, page_selector in (
            ("/how/documentation-system", "main.documentation-page"),
            ("/how/ai-native-engineering", "main.ai-native-page"),
        ):
        ...
        activate_and_capture(
            page,
            "review-queue",
            "03-review-queue-annotation-active.png",
            "대화 흐름과 검토 업무를 분리",
        )
        activate_and_capture(
            page,
            "source-provenance",
            "04-source-annotation-active.png",
            "돌아갈 원문",
        )
        activate_and_capture(
            page,
            "editable-draft",
            "05-draft-annotation-active.png",
            "AI의 결론이 아니라 검토 재료",
        )
        activate_and_capture(
            page,
            "human-decision",
            "06-human-decision-annotation-active.png",
            "한 번 더 충돌을 확인",
        )
        activate_and_capture(
            page,
            "trust-boundary",
            "07-canonical-boundary-annotation-active.png",
            "기본 답변 근거에서 제외",
        )

        page.locator(".acr-workflow-section").screenshot(
            path=str(OUTPUT_DIR / "08-workflow.png")
        )
        page.locator(".acr-evolution").screenshot(
            path=str(OUTPUT_DIR / "09-evolution.png")
        )
        page.locator(".acr-evidence").screenshot(
            path=str(OUTPUT_DIR / "10-evidence-status.png")
        )
        page.locator(".acr-boundary").screenshot(
        ...
        mobile_page.locator('[data-hotspot="human-decision"]').tap()
        mobile_page.locator("#product-editorial-note").evaluate(
            "element => element.scrollIntoView({ block: 'start' })"
        )
        mobile_page.wait_for_timeout(80)
        assert "한 번 더 충돌을 확인" in mobile_page.locator(
