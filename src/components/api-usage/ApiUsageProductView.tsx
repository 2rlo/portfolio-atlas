...
function ApiUsageProductView({ fixture, activeId, onActivate }: ApiUsageProductViewProps) {
  return (
    <section className="api-usage-product" data-has-active={activeId ? 'true' : 'false'} aria-label={`${fixture.productName} API 사용량 공개 재구성 화면`}>
      ...
      <div className="api-usage-canvas">
        <header className="api-usage-head" aria-hidden="true"><div><small>OPERATIONS DESK / ADMIN</small><h2>API 비용</h2><p>AI 기능별 사용량과 계산 비용, 예산 상태를 추적합니다.</p></div><span>{fixture.period}</span></header>
        <nav className="api-usage-tabs" aria-hidden="true"><span>TEAM & PERMISSIONS</span><strong>API COST</strong><span>PROJECTS</span></nav>

        <ProductHotspot id="budget-guard" activeId={activeId} label="작업별 백그라운드 예산 제어" className="api-budget-guard" onActivate={onActivate}>
          <span>04</span><span><small>{fixture.guard.status}</small><strong>채팅은 유지하고, 기준을 넘은 백그라운드 작업은 중단합니다.</strong></span><span><small>WARNING / STOP / PROJECTION</small><b>{fixture.guard.dailyWarning} · {fixture.guard.dailyStop} · {fixture.guard.monthlyProjection}</b></span><span><small>KEPT ONLINE</small><b>{fixture.guard.keptOnline}</b></span><span><small>PAUSED WHEN EXCEEDED</small><b>{fixture.guard.paused}</b></span>
        </ProductHotspot>
        ...
        <div className="api-breakdown-grid">
          <ProductHotspot id="workload-split" activeId={activeId} label="기능과 호출 방식별 비용 집계" className="api-workloads" onActivate={onActivate}>
          ...
          <ProductHotspot id="pricing-dimensions" activeId={activeId} label="구분해서 계산하는 토큰별 요율" className="api-token-ledger" onActivate={onActivate}>
        ...
        <ProductHotspot id="usage-ledger" activeId={activeId} label="호출별 사용량과 연결 중단 기록" className="api-event-ledger" onActivate={onActivate}>
        ...
        <ProductHotspot id="counterfactual" activeId={activeId} label="원장 금액과 다른 요율을 가정한 비용 비교" className="api-counterfactual" onActivate={onActivate}>
