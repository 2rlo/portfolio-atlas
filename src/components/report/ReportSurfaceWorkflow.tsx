...
import type { ReportHotspotId, ReportPageContent } from '../../content/content-types.ts'
...
type StepId = ReportPageContent['workflow']['steps'][number]['id']
type Preview = { id: StepId; kind: 'pointer' | 'focus' | 'tap' } | null

function ReportSurfaceWorkflow({ content }: { readonly content: ReportPageContent }) {
  ...
  const inspection = useProductInspectionState<ReportHotspotId>()
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      stage.style.setProperty('--rptj-expanded-height', `${panel.offsetHeight}px`)
      stage.style.setProperty('--rptj-canvas-width', `${canvas.offsetWidth}px`)
    }
    measure()
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const readable = items.map((item, candidate) => {
        const detail = item.querySelector('.rptj-rail-detail') ?? item.querySelector('.rptj-rail-summary')
  ...
  useLayoutEffect(() => {
    ...
    const frameSurface = () => {
      const mobile = window.matchMedia('(max-width: 56rem)').matches
      const product = canvas.querySelector<HTMLElement>('.report-product')
      if (!product) return
      const scale = mobile
        ? 1
        : Math.min(1, viewport.clientWidth / canvas.offsetWidth, (window.innerHeight - 150) / product.offsetHeight)
      panel.style.setProperty('--rptj-surface-scale', String(scale))
      panel.style.setProperty('--rptj-overview-height', `${product.offsetHeight * scale}px`)
      ...
      panel.style.setProperty('--rptj-crop-offset', `${-offset}px`)
      if (hotspot) {
        const region = hotspot.getBoundingClientRect()
        const frame = viewport.getBoundingClientRect()
        panel.style.setProperty('--rptj-marker-x', `${Math.max(4, Math.min(frame.width - 28, region.left - frame.left - 12))}px`)
        panel.style.setProperty('--rptj-marker-y', `${Math.max(4, Math.min(frame.height - 28, region.top - frame.top - 12))}px`)
      }
    }
    frameSurface()
  ...
  return (
    <section className="rptj-journey" id="rptj-product-journey" aria-labelledby="report-title-inspection">
      <div className="rptj-journey-heading">
        <header className="qa-inspection-heading">
          <p>{content.inspection.eyebrow}</p>
          <h2 id="report-title-inspection">{content.inspection.title}</h2>
          <span>{content.inspection.instruction}</span>
        </header>
      </div>
      <div
        className="rptj-journey-stage"
        ref={stageRef}
        data-compact={compact}
        data-scroll-step={scrollStepId}
        data-active-step={displayedStepId ?? activeWorkflowStep.id}
      >
        <div className="rptj-following-surface" ref={panelRef}>
          <div className="rptj-following-frame" inert={compact} aria-hidden={compact ? true : undefined}>
            ...
            >
              <div className="rptj-surface-viewport">
                <div className="rptj-surface-canvas" ref={canvasRef}>
                  <ReportProductView fixture={content.product} activeId={activeId} onActivate={inspection.activateHotspot} />
                </div>
                {compact ? <span className="rptj-surface-marker" aria-hidden="true">{activeWorkflowStep.index}</span> : null}
              </div>
            </ProductInspectionFrame>
          </div>
          <div
            className="rptj-surface-caption"
            id="rptj-surface-caption"
            hidden={!compact}
        ...
        <div className="rptj-reading-column">
          <header className="rptj-reading-heading" ref={headingRef}>
            <p>{content.workflow.eyebrow}</p>
            <h2 id="report-title-workflow">{content.workflow.title}</h2>
            <span>{content.workflow.introduction}</span>
          </header>
          <ol
            className="rptj-workflow-rail"
            ref={stepsRef}
            aria-labelledby="report-title-workflow"
          ...
          >
            {content.workflow.steps.map((item) => {
              ...
              const detail = item.id === 'report-flow-select'
                ? undefined
                : note?.sections.find((entry) => entry.label === (item.id === 'report-flow-review' ? 'WHY' : 'DECISION'))?.body
              const active = activeWorkflowStep.id === item.id
              return (
                <li key={item.id} id={`rptj-${item.id}`} data-step={item.id} data-active={active}>
                  <h3>
                    <button
                      type="button"
                      aria-current={active ? 'step' : undefined}
                      aria-controls="rptj-surface-caption"
                      aria-describedby={`rptj-summary-${item.id}`}
                    ...
                    >
                      <span className="rptj-rail-marker">{item.index}</span>
                      <span className="rptj-rail-label">{item.label}</span>
                      <span className="rptj-rail-indicator" aria-hidden="true">↗</span>
                    </button>
                  </h3>
                  <p className="rptj-rail-summary" id={`rptj-summary-${item.id}`}>{item.summary}</p>
                  {detail ? <p className="rptj-rail-detail">{detail}</p> : null}
                </li>
              )
            })}
          </ol>
          <p className="rptj-workflow-boundary">{content.workflow.boundary}</p>
        </div>
      </div>
    </section>
  )
...
export default ReportSurfaceWorkflow
