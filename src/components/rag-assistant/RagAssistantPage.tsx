...
import RagAssistantSurfaceWorkflow from './RagAssistantSurfaceWorkflow.tsx'
...
function RagAssistantPage({ content }: { readonly content: RagAssistantPageContent }) {
  const pageClassName = 'rag-assistant-page'
  const titleId = 'rag-assistant-title'
  const evolutionTargetId = 'rag-assistant-evolution'
  const relatedLabel = 'RAG 어시스턴트와 연결된 시스템'
  const renderEvolutionFragment = (visual: RagAssistantEvolutionVisual) => <RagAssistantEvolutionFragment visual={visual} />
  ...
  return (
    <main className={`qa-page ${pageClassName}`} id="main-content">
      ...
      <RagAssistantSurfaceWorkflow content={content} />
