...
import type { RagAssistantHotspotId, RagAssistantPageContent } from '../../content/content-types.ts'
...
type Preview = { id: RagAssistantHotspotId; kind: 'pointer' | 'focus' | 'tap' } | null

function RagAssistantSurfaceWorkflow({ content }: { readonly content: RagAssistantPageContent }) {
  ...
  const inspection = useProductInspectionState<RagAssistantHotspotId>()
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      stage.style.setProperty('--rag-expanded-height', `${panel.offsetHeight}px`)
      stage.style.setProperty('--rag-canvas-width', `${canvas.offsetWidth}px`)
    }
    measure()
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const readable = items.map((item, candidate) =>
        ...
        && item.querySelector('.rag-rail-detail')!.getBoundingClientRect().bottom > readingTop,
      )
  ...
  useLayoutEffect(() => {
    ...
    const frameSurface = () => {
      const mobile = window.matchMedia('(max-width: 56rem)').matches
      const product = canvas.querySelector<HTMLElement>('.rag-product')
      if (!product) return
      const scale = mobile
        ? 1
        : Math.min(1, viewport.clientWidth / canvas.offsetWidth, (window.innerHeight - 150) / product.offsetHeight)
      panel.style.setProperty('--rag-surface-scale', String(scale))
      panel.style.setProperty('--rag-overview-height', `${product.offsetHeight * scale}px`)
      ...
      panel.style.setProperty('--rag-crop-offset', `${-offset}px`)
      if (hotspot) {
        const region = hotspot.getBoundingClientRect()
        const frame = viewport.getBoundingClientRect()
        panel.style.setProperty('--rag-marker-x', `${Math.max(4, Math.min(frame.width - 28, region.left - frame.left - 12))}px`)
        panel.style.setProperty('--rag-marker-y', `${Math.max(4, Math.min(frame.height - 28, region.top - frame.top - 12))}px`)
      }
    }
    frameSurface()
  ...
  return (
    <section className="rag-journey" id="rag-product-journey" aria-labelledby="rag-assistant-title-inspection">
      <div className="rag-journey-heading">
        <header className="qa-inspection-heading">
          <p>{content.inspection.eyebrow}</p>
          <h2 id="rag-assistant-title-inspection">{content.inspection.title}</h2>
          <span>{content.inspection.instruction}</span>
        </header>
      </div>
      <div className="rag-journey-stage" ref={stageRef} data-compact={compact} data-scroll-step={scrollId} data-active-step={activeId}>
        <div className="rag-following-surface" ref={panelRef}>
          <div className="rag-following-frame" inert={compact} aria-hidden={compact ? true : undefined}>
            ...
            >
              <div className="rag-surface-viewport">
                <div className="rag-surface-canvas" ref={canvasRef}>
                  <RagAssistantProductView fixture={content.product} activeId={activeId} onActivate={inspection.activateHotspot} />
                </div>
                {compact && <span className="rag-surface-marker" aria-hidden="true">{activeWorkflowStep.index}</span>}
              </div>
            </ProductInspectionFrame>
          </div>
          <div className="rag-surface-caption" id="rag-surface-caption" hidden={!compact} aria-live={preview && preview.kind !== 'pointer' ? 'polite' : 'off'} aria-atomic="true">
            <span>{activeWorkflowStep.index} / {activeWorkflowStep.label}</span>
          </div>
        </div>

        <div className="rag-reading-column">
          <header className="rag-reading-heading" ref={headingRef}>
            <p>{content.workflow.eyebrow}</p>
            <h2 id="rag-assistant-title-workflow">{content.workflow.title}</h2>
            <span>{content.workflow.introduction}</span>
          </header>
          <ol className="rag-workflow-rail" ref={stepsRef} aria-labelledby="rag-assistant-title-workflow" onKeyDown={(event) => { if (event.key === 'Escape') setPreview(null) }}>
            {content.workflow.steps.map((item) => {
              const note = content.annotations.find((entry) => entry.id === item.hotspotId)
              return (
                <li key={item.id} id={`rag-${item.id}`} data-step={item.hotspotId} data-active={activeId === item.hotspotId}>
                  <h3>
                    <button
                      type="button"
                      aria-current={activeId === item.hotspotId ? 'step' : undefined}
                      aria-controls="rag-surface-caption"
                      aria-describedby={`rag-summary-${item.id}`}
                    ...
                    >
                      <span className="rag-rail-marker">{item.index}</span>
                      <span className="rag-rail-label">{item.label}</span>
                      <span className="rag-rail-indicator" aria-hidden="true">↗</span>
                    </button>
                  </h3>
                  <p className="rag-rail-summary" id={`rag-summary-${item.id}`}>{item.summary}</p>
                  <p className="rag-rail-detail">{note?.sections.find((entry) => entry.label === (item.hotspotId === 'source-hierarchy' ? 'WHY' : 'DECISION'))?.body}</p>
                </li>
              )
            })}
          </ol>
          <p className="rag-workflow-boundary">{content.workflow.boundary}</p>
        </div>
      </div>
    </section>
  )
...
export default RagAssistantSurfaceWorkflow
