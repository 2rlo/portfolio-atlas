...
import PermissionSurfaceWorkflow from './PermissionSurfaceWorkflow.tsx'
...
function PermissionPage({ content }: { readonly content: PermissionPageContent }) {
  const pageClassName = 'permission-page'
  const titleId = 'permission-title'
  const evolutionTargetId = 'permission-evolution'
  const relatedLabel = '권한 관리와 연결된 사례와 시스템'
  const renderEvolutionFragment = (visual: PermissionEvolutionVisual) => <PermissionEvolutionFragment visual={visual} />
  ...
  return (
    <main className={`qa-page ${pageClassName}`} id="main-content">
      ...
      <PermissionSurfaceWorkflow content={content} />
