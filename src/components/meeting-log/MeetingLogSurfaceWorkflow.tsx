...
import type { MeetingLogHotspotId, MeetingLogPageContent } from '../../content/content-types.ts'
...
type StepId = MeetingLogPageContent['workflow']['steps'][number]['id']
type Preview = { id: StepId; kind: 'pointer' | 'focus' | 'tap' } | null

function MeetingLogSurfaceWorkflow({ content }: { readonly content: MeetingLogPageContent }) {
  ...
  const inspection = useProductInspectionState<MeetingLogHotspotId>()
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      stage.style.setProperty('--mlj-expanded-height', `${panel.offsetHeight}px`)
      stage.style.setProperty('--mlj-canvas-width', `${canvas.offsetWidth}px`)
    }
    measure()
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const readable = items.map((item, candidate) => {
        const detail = item.querySelector('.mlj-rail-detail') ?? item.querySelector('.mlj-rail-summary')
  ...
  useLayoutEffect(() => {
    ...
    const frameSurface = () => {
      const mobile = window.matchMedia('(max-width: 56rem)').matches
      const product = canvas.querySelector<HTMLElement>('.meeting-product')
      if (!product) return
      const scale = mobile
        ? 1
        : Math.min(1, viewport.clientWidth / canvas.offsetWidth, (window.innerHeight - 150) / product.offsetHeight)
      panel.style.setProperty('--mlj-surface-scale', String(scale))
      panel.style.setProperty('--mlj-overview-height', `${product.offsetHeight * scale}px`)
      ...
      panel.style.setProperty('--mlj-crop-offset', `${-offset}px`)
      if (hotspot) {
        const region = hotspot.getBoundingClientRect()
        const frame = viewport.getBoundingClientRect()
        panel.style.setProperty('--mlj-marker-x', `${Math.max(4, Math.min(frame.width - 28, region.left - frame.left - 12))}px`)
        panel.style.setProperty('--mlj-marker-y', `${Math.max(4, Math.min(frame.height - 28, region.top - frame.top - 12))}px`)
      }
    }
    frameSurface()
  ...
  return (
    <section className="mlj-journey" id="mlj-product-journey" aria-labelledby="meeting-log-title-inspection">
      <div className="mlj-journey-heading">
        <header className="qa-inspection-heading">
          <p>{content.inspection.eyebrow}</p>
          <h2 id="meeting-log-title-inspection">{content.inspection.title}</h2>
          <span>{content.inspection.instruction}</span>
        </header>
      </div>
      <div
        className="mlj-journey-stage"
        ref={stageRef}
        data-compact={compact}
        data-scroll-step={scrollStepId}
        data-active-step={displayedStepId ?? activeWorkflowStep.id}
      >
        <div className="mlj-following-surface" ref={panelRef}>
          <div className="mlj-following-frame" inert={compact} aria-hidden={compact ? true : undefined}>
            ...
            >
              <div className="mlj-surface-viewport">
                <div className="mlj-surface-canvas" ref={canvasRef}>
                  <MeetingLogProductView fixture={content.product} activeId={activeId} onActivate={inspection.activateHotspot} />
                </div>
                {compact ? <span className="mlj-surface-marker" aria-hidden="true">{activeWorkflowStep.index}</span> : null}
              </div>
            </ProductInspectionFrame>
          </div>
          <div
            className="mlj-surface-caption"
            id="mlj-surface-caption"
            hidden={!compact}
        ...
        <div className="mlj-reading-column">
          <header className="mlj-reading-heading" ref={headingRef}>
            <p>{content.workflow.eyebrow}</p>
            <h2 id="meeting-log-title-workflow">{content.workflow.title}</h2>
            <span>{content.workflow.introduction}</span>
          </header>
          <ol
            className="mlj-workflow-rail"
            ref={stepsRef}
            aria-labelledby="meeting-log-title-workflow"
          ...
          >
            {content.workflow.steps.map((item) => {
              ...
              return (
                <li key={item.id} id={`mlj-${item.id}`} data-step={item.id} data-active={active}>
                  <h3>
                    <button
                      type="button"
                      aria-current={active ? 'step' : undefined}
                      aria-controls="mlj-surface-caption"
                      aria-describedby={`mlj-summary-${item.id}`}
                    ...
                    >
                      <span className="mlj-rail-marker">{item.index}</span>
                      <span className="mlj-rail-label">{item.label}</span>
                      <span className="mlj-rail-indicator" aria-hidden="true">↗</span>
                    </button>
                  </h3>
                  <p className="mlj-rail-summary" id={`mlj-summary-${item.id}`}>{item.summary}</p>
                  {detail ? <p className="mlj-rail-detail">{detail}</p> : null}
                </li>
              )
            })}
          </ol>
          <p className="mlj-workflow-boundary">{content.workflow.boundary}</p>
        </div>
      </div>
    </section>
  )
...
export default MeetingLogSurfaceWorkflow
